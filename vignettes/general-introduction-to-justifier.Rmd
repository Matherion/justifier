...
- `assertion`: An `assertion` is a statement that something is the case. An `assertion` will, if possible, be based on one or more `source`s. In `justifier`, an `assertion` is very similar to `assertion`s in the nanopublication ontology (see https://nanopub.com), in which case one could compare the `assertion`'s `description` plus the referenced `source`s or other `justifer` elements as its provenance.
...
- exhaustive lists of values
- scores, weights, or ranks attached to these values

These specifications can pertain either to `justifier` specifications within a given domain, but they can also hold for all `justifier` specifications. For example, it is possible to specify that all `sources` must have a `type`, and that that type must always be `team belief`, `expert consensus`, `target group consultation`, `qualitative study`, `quantitative study`, or `research synthesis`.

## Efficient specification and scope

When specifying something for multiple justifications, the eponymous `justifier` element can be used, making that the fifth `justifier` element. Contents of a `justifier` element pertain to all other elements in a file (`scope: local`), or in a directory (`scope: global`), or all files that are read by the `justifier` package (`scope: universal`).

For example, to specify that the `justification framework` is stored in file `justifier-framework-specification.jfs`, and that this framework should be applied to all justifications in this directory that do not have another framework attached:

```
justifier:
  framework: justifier-framework-specification.jfs
  scope: global
```

To specify that all justificiations in this file are produced at date the sixth of March 2019:
